...
class ChessAI():
    ...
    def get_eval_bar(self,board, curTurn, move_object_moves):
        ...
        return evaluation
    async def recursive_async_helper_white (self,curBoard,curTurn,curDepth, alpha, beta, move_object_moves):
        #####################
        # ORIGINAL VAL CALC #
        #####################
    ...
    async def recursive_async_helper_black (self,curBoard,curTurn,curDepth, alpha, beta, move_object_moves):
        #####################
        # ORIGINAL VAL CALC #
        #####################
        minmaxVal = self.minimax_recursive(curBoard,not curTurn,curDepth+1, alpha, beta, move_object_moves)
    ...
    async def minimax_recursive(self,curBoard,curTurn,curDepth, alpha, beta, move_object_moves):
        ...
        if curTurn == chess.WHITE:
            ...
            tasks = []
            for move in curBoard.legal_moves:
                curBoard.push(move)
                tasks.append(self.recursive_async_helper_white(curBoard,curTurn,curDepth, alpha, beta, move_object_moves))
                curBoard.pop()
    ...
    def get_ai_move(self,board,turn):
        ...
        best_outcome = asyncio.run(self.minimax_recursive(board,turn,0, float(-math.inf), float(math.inf), move_object_moves))
