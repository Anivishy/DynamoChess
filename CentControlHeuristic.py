...
class CenterControlClass:
    ...
    # def calculate_score(move, board, center_position):
    ...
    def post_async(self, board, ccc, move):
        center_moves = ["c3", "c4", "c5", "c6", "d3", "d4", "d5", "d6", "e3", "e4", "e5", "e6", "f3", "f4", "f5", "f6"]
        move_score = 0
        # calculate_score_partial = partial(ccc.calculate_score, move, board)
        # with Pool(2) as p:
        #     move_score += p.map(calculate_score_partial, center_moves)
    ...
    def calc_score(self, moves: list, board, ccc):
        control_score = 0
        post_async_partial = partial(ccc.post_async, board, ccc)
        with Pool() as p:
            control_score += sum(p.map(post_async_partial, moves))
        print(control_score)
    ...
    def centerControl (self, board: chess.Board, move_object_moves):
        ...
        final_score = ccc.calc_score(move_object_moves, board, ccc)
        print(final_score)
        return final_score
